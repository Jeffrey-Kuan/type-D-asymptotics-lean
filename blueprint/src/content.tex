...
statements, not hidden assumptions. At the time of writing the single
remaining \texttt{sorry} is the Mitoma citation (its supporting theory is
complete; see the final chapter).
...
%   \leanok      — the declaration is sorry-free in the current build.  The project's ONE
%                  remaining sorry is thm_mitoma (TypeDDecouplingEW.lean); its entry below
%                  carries no \leanok.
...
\begin{theorem}[Mitoma's tightness criterion]\label{thm:ew-mitoma}
  \lean{TypeDDecoupling.thm_mitoma}
  \uses{def:sk-space, thm:mit-confinement}
  A sequence of c\`adl\`ag $\mathcal S'(\mathbb R)$-valued processes is tight in
  $D([0,T];\mathcal S'(\mathbb R))$ iff, for every $\varphi \in \mathcal S(\mathbb R)$,
  the real-valued sequence $(Z^N(\varphi,\cdot))$ is tight in $D([0,T];\mathbb R)$.
  Stated over the opaque objects \texttt{SchwartzDistModel}, \texttt{distTight},
  \texttt{realTight}, \texttt{mitomaEval}.  \emph{This is the one remaining citation
  sorry of the project; see the Mitoma campaign chapter for the machinery built to
  discharge it} (nuclear confinement on the Hermite scale, \ref{thm:mit-confinement}).
\end{theorem}
...
\chapter{The Skorokhod space $D([0,1],\mathbb{R})$}

\section{The space and Billingsley's metric $d^\circ$}
...
\section{The c\`adl\`ag modulus $w'$}
...
\section{Measurability: evaluations and the cylinder $\sigma$-algebra}
...
\chapter{Nuclear structure of $\mathcal{S}(\mathbb{R})$ and Mitoma's criterion}

\section{The Fréchet package for $\mathcal{S}(\mathbb{R})$}
...
\section{Hermite functions and the Hilbert basis of $L^2(\mathbb{R})$}
